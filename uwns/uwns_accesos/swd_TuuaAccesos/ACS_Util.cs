...
namespace LAP.TUUA.ACCESOS
{
    ...
    class ACS_Util
    {
        ...
        public bool VerificarUsuarioDestrabe(string strCodUsuario, DataTable dtSupervisor, ref string strCuenta, bool isConexionRemoto)
        {
            String sCheckDestrabe = null;
                ...
                else
                {
                    for (int i = 0; i < dtSupervisor.Rows.Count; i++)
                    {
                        if (dtSupervisor.Rows[i].ItemArray.GetValue(1).ToString() == strCodUsuario)
                        {
                            ...
                            sCheckDestrabe = dtSupervisor.Rows[i].ItemArray.GetValue(25).ToString();

                            if (isConexionRemoto && sCheckDestrabe != null && sCheckDestrabe.Equals("1"))
                            {
                                isResultado = true;
                            }
                            break;
        ...
        //Agreago por cmontes 02/09/2015
        /// <summary>
        /// Obtiene confirmación si el usuario tiene habilitado el destrabe 
        /// </summary>
        /// <param name="strAirlineDesign">Codigo de Usauario</param>
        /// <param name="lstCompania">Lista de Usarios </param>
        /// <returns>true en caso que exista el Usuario</returns>
        //public bool VerificarUsuario(string strCodUsuario, List<Usuario> lstUsuario)
        public bool VerificarUsuarioCambioTVMolinete(string strCodUsuario, DataTable dtSupervisor, ref string strCuenta, bool isConexionRemoto)
        {
            String sCheckCambioTVMolinete=null;
            bool isResultado = false;
            if (isConexionRemoto)
            {
                if (dtSupervisor == null)
                ...
                for (int i = 0; i < dtSupervisor.Rows.Count; i++)
                {
                    if (dtSupervisor.Rows[i].ItemArray.GetValue(1).ToString() == strCodUsuario){
                        strCuenta = dtSupervisor.Rows[i].ItemArray.GetValue(9).ToString();
                        sCheckCambioTVMolinete = dtSupervisor.Rows[i].ItemArray.GetValue(27).ToString();
                        if (isConexionRemoto && sCheckCambioTVMolinete != null && sCheckCambioTVMolinete.Equals("1"))
                        {
                            isResultado = true;
                        }
                    }
        ...
        public String ValidarVueloProgramado(VueloProgramado objVueloProg, string strUmbralHora,string strTipDemorado)
        {
            ...
            if (objVueloProg.Tip_Estado.Equals(strTipDemorado))
            {
                ...
                {
                    strFecVuelo = strFecVuelo + "000000";
                }
                else
                {
                    strFecVuelo = strFecVuelo + objVueloProg.Hor_Vuelo;
                }
                dUmbral = Double.Parse(strUmbralHora);
                dtVuelo = DateTime.ParseExact(strFecVuelo, "yyyyMMddHHmmss", new DateTimeFormatInfo());

                dtVueloSup = dtVuelo.AddHours(dUmbral);

                strVueloSup = dtVueloSup.ToString("yyyyMMddHHmmss", CultureInfo.InvariantCulture);
            }
            else
            {
