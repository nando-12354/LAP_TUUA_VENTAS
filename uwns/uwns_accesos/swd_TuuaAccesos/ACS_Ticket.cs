...
namespace LAP.TUUA.ACCESOS
{
    class ACS_Ticket
    {
        ...
        public ACS_Ticket(ACS_InterfazPinPad objIntzPinPad, List<ListaDeCampo> Lst_ListaDeCampo, List<TipoTicket> Lst_TipoTicket, ACS_Resolver Obj_Resolver,
        ...
        {
            this.Obj_IntzPinPad = objIntzPinPad;
        ...
        private void MostrarMsgConcurrente()
        {
            if (ACS_Property.estadoPinPad)
            {
                int Rpta = Obj_IntzPinPad.MostrarMsgPINPad(ACS_Define.Cod_RegTickOk, ACS_Define.Msg_RegTickOk, "");
                //Thread thMes
                if (Rpta == 2)
                {
                    //ESilva - Comentado a solicitud de RSalazar - 27-07-2010
            ...
            try
            {
                if (ValidarDuplicidadTICKETOffLine(objTicket))
                {
                    ...
                    if (ACS_Property.estadoMolinete)
                    {
                        ...
                        {
                            bRptaMolinete = Obj_MolineteDiscapacitados.MolineteDiscapacitadosAprobadoNormal();
                            bRptaMolinete = true;
                        }
                    }
                    if (numReintentos > 1)
                            ...
                            objTicket.STipEstadoActual.Trim().Equals(ACS_Define.Cod_EstTicPremitido))
                        {
                            ...
                            if (strFechaProgramado.Length == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if (((string)ACS_Property.shtMolinete["Tip_Molinete"]).ToUpper().Equals("NORMAL"))
                                            bRptaMolinete = Obj_Molinete.MolineteAprobadoEspecial();
                                        if (((string)ACS_Property.shtMolinete["Tip_Molinete"]).ToUpper().Equals("DISCAPACITADO"))
                                        {
                                            bRptaMolinete = Obj_MolineteDiscapacitados.MolineteDiscapacitadosAprobadoEspecial();
                                            bRptaMolinete = true;
                                        }
                                    }
