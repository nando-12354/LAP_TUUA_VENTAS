...
namespace LAP.TUUA.ACCESOS
{
    class ACS_Boarding
    {
        ...
        public ACS_Boarding(Hashtable htBoarding, List<Compania> Lst_Compania, List<ListaDeCampo> Lst_ListaDeCampo,
            ACS_Util Obj_Util, ACS_Resolver Obj_Resolver, ACS_FormContingenciaDesktop frmContingencia)
        {
        ...
        private string ValidarBCBPRelacionado(BoardingBcbp Obj_BoardingBcbp)
        {
            if (Obj_BoardingBcbp.Num_Secuencial_Bcbp_Rel > 0)
            {
                ...
                BoardingBcbp objBCBPBase = Obj_Resolver.obtenerDAOBoardingBcbp(null, null, null, null, null,
                    Obj_BoardingBcbp.Num_Secuencial_Bcbp_Rel.ToString(), null);
            ...
            VueloProgramado ObjVueloProgramado = Obj_Resolver.obtenerVueloProgramado(
                Obj_BoardingBcbp.Num_Vuelo_Rel.Substring(0, 8), Obj_BoardingBcbp.Num_Vuelo_Rel.Substring(8));
        ...
        public void ValidarBCBP(BoardingBcbp Obj_BoardingBcbp, String strTrama)
        {
                ...
                ValidarDuplicidadBCBPOffLine(Obj_BoardingBcbp)) //LAP-TUUA-9163 - 15-06-2012 (RS) CMONTES
            {
                    ...
                    strOrigenAerop != (string)Property.htProperty[ACS_Define.FROM_CITY_AIRPORT_CODE.ToUpper()])
                {
                    ...
                    MostrarError(ACS_Define.Cod_RegBCBPErr, strTrama, ACS_Define.Cod_ErrBCBPFecha,
                        strDscError.ToUpper());
                    return;
                }
                else
                ...
                VueloProgramado ObjVueloProgramado = Obj_Resolver.obtenerVueloProgramado(Obj_BoardingBcbp.StrFchVuelo,
                    (String)Ht_Boarding["flight_number"]);

                if (ObjVueloProgramado != null || flagRehab ||
                    !ACS_Property.BConRemota) //LAP-TUUA-9163 - 15-06-2012 (RS) CMONTES
                {
                    ...
                    Obj_Util.EscribirLog(this,
                        "TIPO VUELO:" + strTipVuelo + " - ObjvueloProgram:" + ObjVueloProgramado.Tip_Vuelo);
                    ...
                    if (strTipVueloMol != strTipVuelo)
                    {
                        ...
                        //GeneraAlarma
                        if (ACS_Property.BConRemota)
                            GestionAlarma.Registrar(ACS_Define.Dsc_SPConfig, "W0000080", "M01", IpClient, "2",
                                "Alerta W0000080",
                                "ERROR EN REGISTRO BOARDING - Codigo Error: " + ACS_Define.Cod_ErrBCBPNoValido +
                                ", Descripcion Error: " + strDscError + ", Ip Servicio: " + IpClient,
                                ACS_Define.Usr_Acceso);
                        return;
                    }
                    ...
                    if (strCodComp.Length > 0)
                    {
                        ...
                        ModVentaComp ObjModaVentaComp =
                            Obj_Resolver.obtenerModVentaComp(strCodComp, ACS_Define.Dsc_ModBCBP);
                        if (ObjModaVentaComp != null)
                        {
                            string strFechaProgramado = string.Empty;
                            if (ACS_Property.BConRemota)
                            {
                                strFechaProgramado = flagRehab
                                    ? ""
                                    : Obj_Util.ValidarVueloProgramado(ObjVueloProgramado,
                                        (string)Property.htProperty["UMBRAL_HORAS"],
                                        (string)Property.htProperty["ESTADO_DEMORADO"]);
                            }
                            else
                            {
                                strFechaProgramado =
                                    "0"; //CMONTES 05/10/2012 para forzar validacion de vigencia de fecha
                            }

                            if ((strFechaProgramado.Trim().Length < 1) || flagRehab ||
                                Obj_Util.ValidarVueloExpiradoOffline(Obj_BoardingBcbp
                                    .StrFchVuelo)) //LAP-TUUA-9163 - 04-10-2012 (RS) CMONTES
                            {
                                ...
                                if (!flagRehab)
                                {
                                    ...
                                    if (Obj_BoardingBcbp.Flg_Val_WS)
                                    {
                                        ...
                                        System.Threading.Thread.Sleep(
                                            (!(bool)Lst_WSBcbp[ACS_Define.KEY_WS_RESPONSE]) &&
                                            (intSleep - intProcesoVal > 0)
                                                ? intSleep - intProcesoVal
                                                : 0);
                                        ...
                                        if ((bool)Lst_WSBcbp[ACS_Define.KEY_WS_RESPONSE])
                                        {
                                            ...
                                            Obj_BoardingBcbp.Dsc_Observacion = "WEB SERVICE - " +
                                                                               Lst_WSBcbp[
                                                                                       ACS_Define.KEY_WS_TRAMA_RESPONSE]
                                                                                   .ToString();
                                            ...
                                            {
                                                Obj_BoardingBcbp.Tip_Transferencia =
                                                    Obj_BoardingBcbp.Flg_Transito.Equals(ACS_Define.FLG_ACTIVO)
                                                        ? LAP.TUUA.UTIL.Define.TRANSITO
                                                        : LAP.TUUA.UTIL.Define.NORMAL;
                                            }
                                            ...
                                            if (Obj_BoardingBcbp.Flg_Val_Tuua)
                                            {
                                                if (Obj_BoardingBcbp.Flg_Pago.Equals(ACS_Define.FLG_INACTIVO))
                                                {
                                                    ...
                                                    if (Obj_BoardingBcbp.Flg_Val_Transito &&
                                                        !Obj_BoardingBcbp.Flg_Transito.Equals(ACS_Define.FLG_ACTIVO))
                                                    {
                                                    ...
                                                    strDscError = "ERROR DE BOARDING;;," + strDscError;
                                                    MostrarError(ACS_Define.Cod_RegBCBPErr, strTrama,
                                                        ACS_Define.Cod_ErrBCBPFecha, strDscError.ToUpper());
                                                    return;
                                                }
                                            }
                                            ...
                                            if (!Obj_BoardingBcbp.Flg_Val_Tuua && Obj_BoardingBcbp.Flg_Val_Transito)
                                            {
                                                if (Obj_BoardingBcbp.Flg_Transito.Equals(ACS_Define.FLG_ACTIVO))
                                                {
                                                    ...
                                                    strDscError = "ERROR DE BOARDING;;," + strDscError;
                                                    MostrarErrorEspecial(ACS_Define.Cod_RegBCBPErr, strTrama,
                                                        ACS_Define.Cod_ErrBCBPFecha, strDscError.ToUpper());
                                                    return;
                                                }
                                            }
                                        ...
                                        {
                                            Obj_BoardingBcbp.Dsc_Observacion =
                                                Lst_WSBcbp[ACS_Define.KEY_WS_ERROR_MSG].ToString();
                                        }
                                    }
                                    else if (Obj_BoardingBcbp.Flg_Val_Transito &&
                                             Obj_BoardingBcbp.Tip_Transferencia.Equals(LAP.TUUA.UTIL.Define.TRANSITO))
                                    {
                                        string strDscError = "PASAJERO EN TRANSITO";
                                        strDscError = "ERROR DE BOARDING;;," + strDscError;
                                        MostrarErrorEspecial(ACS_Define.Cod_RegBCBPErr, strTrama,
                                ...
                                int numMaxReintentos =
                                    Convert.ToInt32((string)Property.htProperty["NUM_REINTENTOS_BP"]);
                                int numReintentos = 0;
                                bool registroBP = false;
                                Obj_Util.EscribirLog(this, "Inicio Registro BP");
                                for (int i = 0; i < numMaxReintentos; i++)
                                {
                                    Obj_Util.EscribirLog(this, "Inicio Registro BP - 1");
                                    Obj_Resolver.CrearDAOBoardingBcbp();
                                    Obj_Util.EscribirLog(this, "Inicio Registro BP - 1.1");
                                    if (Obj_Resolver.insertarDAOBoardingBcbp(Obj_BoardingBcbp))
                                    {
                                        Obj_Util.EscribirLog(this, "Inicio Registro BP - 2");
                                        if (numReintentos > 1)
                                            Obj_Util.EscribirLog(this,
                                                "REGISTRO OK (" + numReintentos + " REINTENTOS)");
                                        //else
                                    ...
                                    System.Threading.Thread.Sleep(100);
                                    Obj_Util.EscribirLog(this, "Inicio Registro BP - 3");
                                }

                                if (!registroBP)
                                {
                                    string strDscError =
                                        "ERROR REGISTRO BP;;REINTENTE,Error al registrar boarding en base de datos";
                                    MostrarError(strTrama, ACS_Define.Cod_ErrBCBPNoReg, strDscError.ToUpper());
                                    //GeneraAlarma
                                    if (ACS_Property.BConRemota)
                                        GestionAlarma.Registrar(ACS_Define.Dsc_SPConfig, "W0000080", "M01", IpClient,
                                            "2", "Alerta W0000080",
                                            "ERROR EN REGISTRO BOARDING - Codigo Error: " +
                                            ACS_Define.Cod_ErrBCBPNoReg + ", Descripcion Error: " + strDscError +
                                            ", Ip Servicio: " + IpClient, ACS_Define.Usr_Acceso);
                                    return;
                                }
                                ...
                                //************************LUZ VERDE****************************************//
                                System.Threading.Thread thMsg =
                                    new System.Threading.Thread(
                                        new System.Threading.ThreadStart(MostrarMsgConcurrente));
                                thMsg.Start();
                            ...
                            {
                                string strDscError = Obj_Util.ObtenerDscListaDeCampos(ACS_Define.Cod_ErrBCBPFecha,
                                    Lst_ListaDeCampo, "ErrorBCBP");
                                strDscError = "ERROR DE BOARDING;;," + strDscError;
                                MostrarError(ACS_Define.Cod_RegBCBPErr, strTrama, ACS_Define.Cod_ErrBCBPFecha,
                                    strDscError.ToUpper());
                                //GeneraAlarma
                                if (ACS_Property.BConRemota)
                                    GestionAlarma.Registrar(ACS_Define.Dsc_SPConfig, "W0000080", "M01", IpClient, "2",
                                        "Alerta W0000080",
                                        "ERROR EN REGISTRO BOARDING - Codigo Error: " + ACS_Define.Cod_ErrBCBPMod +
                                        ", Descripcion Error: " + strDscError + ", Ip Servicio: " + IpClient,
                                        ACS_Define.Usr_Acceso);
                            }
                        }
                        ...
                        {
                            string strDscError = Obj_Util.ObtenerDscListaDeCampos(ACS_Define.Cod_ErrBCBPMod,
                                Lst_ListaDeCampo, "ErrorBCBP");
                            strDscError = "ERROR DE BOARDING;;," + strDscError;
                            MostrarError(ACS_Define.Cod_RegBCBPErr, strTrama, ACS_Define.Cod_ErrBCBPMod,
                                strDscError.ToUpper());
                            //GeneraAlarma
                            if (ACS_Property.BConRemota)
                                GestionAlarma.Registrar(ACS_Define.Dsc_SPConfig, "W0000080", "M01", IpClient, "2",
                    ...
                    {
                        string strDscError = Obj_Util.ObtenerDscListaDeCampos(ACS_Define.Cod_ErrBCBPAero,
                            Lst_ListaDeCampo, "ErrorBCBP");
                        strDscError = "ERROR DE BOARDING;;," + strDscError;
                        MostrarError(ACS_Define.Cod_RegBCBPErr, strTrama, ACS_Define.Cod_ErrBCBPAero,
                            strDscError.ToUpper());
                        //GeneraAlarma
                        if (ACS_Property.BConRemota)
                            GestionAlarma.Registrar(ACS_Define.Dsc_SPConfig, "W0000080", "M01", IpClient, "2",
                                "Alerta W0000080",
                                "ERROR EN REGISTRO BOARDING - Codigo Error: " + ACS_Define.Cod_ErrBCBPAero +
                                ", Descripcion Error: " + strDscError + ", Ip Servicio: " + IpClient,
                                ACS_Define.Usr_Acceso);
                    }
                }
                ...
                {
                    string strDscError =
                        Obj_Util.ObtenerDscListaDeCampos(ACS_Define.Cod_ErrBCBVuelo, Lst_ListaDeCampo, "ErrorBCBP");
                    strDscError = "ERROR DE BOARDING;;," + strDscError;
                    MostrarError(ACS_Define.Cod_RegBCBPErr, strTrama, ACS_Define.Cod_ErrBCBVuelo,
                        strDscError.ToUpper());
                    //GeneraAlarma
                    if (ACS_Property.BConRemota)
                        GestionAlarma.Registrar(ACS_Define.Dsc_SPConfig, "W0000080", "M01", IpClient, "2",
                            "Alerta W0000080",
                            "ERROR EN REGISTRO BOARDING - Codigo Error: " + ACS_Define.Cod_ErrBCBVuelo +
                            ", Descripcion Error: " + strDscError + ", Ip Servicio: " + IpClient,
                            ACS_Define.Usr_Acceso);
                }
            }
            else
            {
                ...
                {
                    strDscError =
                        Obj_Util.ObtenerDscListaDeCampos(ACS_Define.Cod_ErrBCBPReg, Lst_ListaDeCampo, "ErrorBCBP");
                    //LAP-TUUA-9163 - 20-06-2012 (RS) CMONTES - BEGIN
                    string strFecha = " " + ACS_Util.ConvertirFecha(Obj_BoardingBcbp.StrLogFechaMod) + " " +
                                      ACS_Util.ConvertirHora(Obj_BoardingBcbp.StrLogHoraMod);
                ...
                if (ACS_Property.BConRemota)
                    GestionAlarma.Registrar(ACS_Define.Dsc_SPConfig, "W0000080", "M01", IpClient, "2",
                        "Alerta W0000080",
                        "ERROR EN REGISTRO BOARDING - Codigo Error: " + ACS_Define.Cod_ErrBCBPReg +
                        ", Descripcion Error: " + strDscError + ", Ip Servicio: " + IpClient, ACS_Define.Usr_Acceso);
            }
        }
        ...
        public void MostrarError(string strCodTransaccion, string strTrama, string strTipoError, string strMsg)
        {
            ...
            try
            {
                ...
                Obj_BoardingBcbpErr.Cod_Compania =
                    Obj_Util.ObtenerCodCompania((String)Ht_Boarding[ACS_Define.AIRLINE_DESIGNATOR], Lst_Compania);
        ...
        private void CargarAtributosWSEntidadBcbp(BoardingBcbp objBcbp)
        {
            if (Lst_WSBcbp != null)
            {
                objBcbp.Flg_Val_WS = Lst_WSBcbp[ACS_Define.ID_VAL_WS] != null && Lst_WSBcbp[ACS_Define.ID_VAL_WS]
                    .ToString().Trim().Equals(ACS_Define.FLG_SI_PAGO_TUUA)
                    ? true
                    : false;
                objBcbp.Flg_Val_Tuua =
                    Lst_WSBcbp[ACS_Define.ID_VAL_PAGO_TUUA] != null && Lst_WSBcbp[ACS_Define.ID_VAL_PAGO_TUUA]
                        .ToString().Trim().Equals(ACS_Define.FLG_SI_PAGO_TUUA)
                        ? true
                        : false;
                objBcbp.Flg_Val_Transito =
                    Lst_WSBcbp[ACS_Define.ID_VAL_TRANSITO] != null && Lst_WSBcbp[ACS_Define.ID_VAL_TRANSITO].ToString()
                        .Trim().Equals(ACS_Define.FLG_SI_PAGO_TUUA)
                        ? true
                        : false;
            }
            else
        ...
        private void VerificarPassengerStatus(BoardingBcbp objBoarding)
        {
            ...
            if (strPassStatus !=
                null) //Modificacion por problema de registro manual de boarding en pinpad 24/03/2011 eochoa
            {
                if (Lst_WSBcbp[ACS_Define.ID_VAL_TRANSITO] != null &&
                    Lst_WSBcbp[ACS_Define.ID_VAL_TRANSITO].ToString().Trim().Equals(ACS_Define.FLG_ACTIVO) &&
                    strPassStatus != "" && strPassStatus.Equals(strStatusTransito))
                {
