...
namespace LAP.TUUA.CONVERSOR
{
    public class Reader
    {
        ...
        //Accesos
        public Hashtable ParseString_ACS(String strTrama, String strPrefijo, string strPreDestrabe)
        {
            if (strTrama != null && !strTrama.Trim().Equals("") && strTrama.Length > 4)
            {
                String cabecera = strTrama.Substring(4, 1);
                ...
                else if (strTrama.Length >= (Define.LongSupervisor + 4) && !strTrama.Substring(4, 3).Equals(strPreDestrabe))
                {
        ...
        public Hashtable Parse_Boarding_Multiple_ACS(String strTrama)
        {
            ...
            try
            {
                ...
                htBoarding.Add("flight_information", ArrLst);
                if (htFlightInformation["field_size_individual_use"].ToString().Trim() != "0")
                {
                    foreach (XmlNode xmlCampo in lstCampo3)
                    {
                        if (strTrama != "")
        ...
        public void AnalizarSegmentosBCBP(ArrayList arrLst,string strFromCityAirportCode)
        {
            string strFecVuelo = "";
            string strNumVuelo = "";
            ...
            for (int i = 0; i < arrLst.Count; i++)
            {
                ...
                if (htSegmento[Define.FROM_CITY_AIRPORT_CODE].Equals(strFromCityAirportCode))
                {
                    strFecVuelo = (string)htSegmento[Define.FechaVuelo];
                    strNumVuelo = (string)htSegmento[Define.NroVuelo];
                    strSeatNumber = (string)htSegmento[Define.Asiento];
                    strAirDesignator = (string)htSegmento[Define.Compania];
                    ((Hashtable)arrLst[0])[Define.FechaVuelo] = strFecVuelo;
                    ((Hashtable)arrLst[0])[Define.NroVuelo] = strNumVuelo;
                    ((Hashtable)arrLst[0])[Define.Asiento] = strSeatNumber;
                    ((Hashtable)arrLst[0])[Define.Compania] = strAirDesignator;
                    return;
                }
            }
        }
