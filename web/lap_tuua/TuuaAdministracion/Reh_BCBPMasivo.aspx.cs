...
public partial class Reh_BCBPMasivo : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                btnRehabilitar.Text = htLabels["rehabilitacionBoardingMasivo.btnRehabilitar"].ToString();
                lblCia.Text = htLabels["rehabilitacionBoardingMasivo.lblCia"].ToString();
                lblFechaVuelo.Text = htLabels["rehabilitacionBoardingMasivo.lblFechaVuelo"].ToString();
                lblVuelo.Text = htLabels["rehabilitacionBoardingMasivo.lblVuelo"].ToString();
                lblAsiento.Text = htLabels["rehabilitacionBoardingMasivo.lblAsiento"].ToString();
                lblPersona.Text = htLabels["rehabilitacionBoardingMasivo.lblPersona"].ToString();
                lblMotivo.Text = htLabels["rehabilitacionBoardingMasivo.lblMotivo"].ToString();
                lblTotalSeleccionados.Text = htLabels["rehabilitacionBoardingMasivo.lblTotalSeleccionados"].ToString();
                lblTotalIngresados.Text = htLabels["rehabilitacionBoardingMasivo.lblTotalIngresados"].ToString();
                lblConformidad.Text = htLabels["rehabilitacionBoardingMasivo.lblConformidad"].ToString();
                rowDesc_Cia.Value = htLabels["rehabilitacionBoardingMasivo.rowDesc_Cia"].ToString();
                rowDesc_NumVuelo.Value = htLabels["rehabilitacionBoardingMasivo.rowDesc_NumVuelo"].ToString();
                rowDesc_FechaVuelo.Value = htLabels["rehabilitacionBoardingMasivo.rowDesc_FechaVuelo"].ToString();
                rowDesc_Asiento.Value = htLabels["rehabilitacionBoardingMasivo.rowDesc_Asiento"].ToString();
                rowDesc_Pasajero.Value = htLabels["rehabilitacionBoardingMasivo.rowDesc_Pasajero"].ToString();
                ...
                if (activarConsRepre == 1)
                {
                    ...
                    lblConsRepresentante.Text = htLabels["rehabilitacionBoardingMasivo.lblConsRepresentante"].ToString();
                }
                else
                {
                ...
                if (dt_parametro.Rows.Count > 0)
                {
                    ...
                    {
                        gwvRehabilitarPorBCBP.Columns[3].Visible = true;
                        gwvRehabilitarPorBCBP.Columns[4].Visible = true;
                        hdMostrarAsociacion.Value = "2";
                    }
                    else
                    {
                        gwvRehabilitarPorBCBP.Columns[3].Visible = false;
    ...
    protected void btnAgregar_Click(object sender, EventArgs e)
    {
        ...
        for (int i = (pageIndex * pageSize), j = 0; j < dtBCBPRehabilitados.Rows.Count - (pageIndex * pageSize); i++, j++)
        {
            ...
            //{
            //    CheckBox chkSeleccionar = (CheckBox)gwvRehabilitarPorBCBP.Rows[j].FindControl("chkSeleccionar");
            //    chkSeleccionar.Checked = (Boolean)dtBCBPRehabilitados.Rows[i]["Check"];//Esto genera error!!!
            //} 
    ...
    protected void btnAgregar_Click2(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (dtBoardingDetalle.Rows.Count == 1)
            {
                ...
                {
                    lblErrorMsg.Text = "El Boarding Pass " + cod_numero_bcbp + " esta vencido. Fecha Vencimiento: " + fec_vencimiento + " - Dias vencidos: " + Di_Vncidos_Log_fec; return;
                }
    ...
    protected void btnRehabilitar_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            {
                System.Threading.Thread.Sleep(500);
                spnRehabilitar.Text = "Ocurrio un error en el proceso de Rehabilitacion ";
                return;
            }
            else
            {
                ...
                GestionAlarma.Registrar(HttpContext.Current.Server.MapPath(""), "W0000045", "005", IpClient, "1", "Alerta W0000045", "Proceso de Rehabilitacion de Boarding-Masivo, completado correctamente, Usuario: " + Convert.ToString(Session["Cod_Usuario"]), Convert.ToString(Session["Cod_Usuario"]));
    ...
    #region btnRehabilitar_Click2
    protected void btnRehabilitar_Click2(object sender, EventArgs e)
    {
    ...
    protected void btnReporte_click(object sender, ImageClickEventArgs e)
    {
        ...
        String url = "ReporteREH/ReporteREH_BCBP.aspx?titulo=Rehabilitacion BCBP Masivo";
            ...
            for (int i = 0; i < dtTicketRehabilitados.Rows.Count; i++)
            {
                ...
                if (chkSeleccionar.Checked)
                {
                    if (flag == 1)
                    {
                        ...
                        dr["Motivo"] = this.cboMotivo.SelectedItem.Text.Trim();
    ...
    private void Imprimir(Hashtable htPrintData)
    {
        ...
        XmlElement nodo = objPrint.ObtenerNodo((XmlDocument)Session["xmlDoc"], Define.ID_PRINTER_DOCUM_REHABILITACIONMASIVA);

        // configuracion de la impresora a utilizar
        string configImpVoucher = objPrint.ObtenerConfiguracionImpresora(nodo, (Hashtable)Session["htParamImp"], Define.ID_PRINTER_DOCUM_REHABILITACIONMASIVA);
        ...
        Response.Redirect("Ope_Impresion.aspx?" +
            ...
            "&" + "Pagina_PreImpresion=Reh_BCBPMasivo.aspx", false);
