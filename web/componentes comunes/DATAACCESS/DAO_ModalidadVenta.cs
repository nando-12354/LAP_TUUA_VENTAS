...
namespace LAP.TUUA.DAO
{
    public class DAO_ModalidadVenta : DAO_BaseDatos
    {
        ...
        public DAO_ModalidadVenta(Hashtable htSPConfig)
            : base(htSPConfig)
        {
            objListaModalidadVenta = new List<ModalidadVenta>();
        }
        ...
        public List<ModalidadVenta> listar()
        {
            ...
            {
                ModalidadVenta ModalVent = poblar2(objResult);
                if (ModalVent != null)
                {
                    objListaModalidadVenta.Add(ModalVent);
                }
            }
            objResult.Dispose();
            objResult.Close();
        ...
        /// </summary>
        public ModalidadVenta poblar2(IDataReader dataReader)
        {
            Hashtable htSelectAllUSP = (Hashtable)htSPConfig["MVSP_SELECTALL"];
            ModalidadVenta objModalidadVenta = null;

            if ((string)dataReader[(string)htSelectAllUSP["Tip_Estado"]] == "1")
            {
                objModalidadVenta = new ModalidadVenta();
