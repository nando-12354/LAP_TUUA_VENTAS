    ...
    public class DAO_LogOperacion : DAO_BaseDatos
    {
        ...
        public bool insertar(LogOperacion objLogOperacion)
        {
            try
            {
                ...
                helper.AddOutParameter(objCommandWrapper, (string)hsInsertUSP["Cod_Operacion"], DbType.String, 10);
                isRegistrado = base.mantenerSP(objCommandWrapper);
                if (isRegistrado)
                {
                    objLogOperacion.SNumOperacion = (string)helper.GetParameterValue(objCommandWrapper, (string)hsInsertUSP["Cod_Operacion"]);
                }
                return isRegistrado;
            }
            catch (Exception)
        ...
        public DataTable obtenerOperacionCompraVenta(string strTipo, string strFecIni, string strHorIni, string strFecFin, string strHorFin, string strTurnoIni, string strTurnoFin)
        {
            try
            {
                DataTable result = null;
                Hashtable hsSelectByIdUSP = (Hashtable)htSPConfig["OPERACIONSP_RESUMENCOMPRAVENTA"];
                string sNombreSP = (string)hsSelectByIdUSP["OPERACIONSP_RESUMENCOMPRAVENTA"];
                if (strTipo != null && strTipo.Length > 0)
                {
                    //if (strTipo == "1" || strTipo == "3")
                        result = base.ListarDataTableSP(sNombreSP, strTipo, strFecIni, strHorIni, strFecFin, strHorFin, strTurnoIni, strTurnoFin);
                    //else
