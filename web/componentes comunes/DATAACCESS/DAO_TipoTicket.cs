    ...
    public class DAO_TipoTicket : DAO_BaseDatos
    {
        ...
        public TipoTicket ObtenerPrecioTicket(string strTipoVuelo, string strTipoPas, string strTipoTrans)
        {
            ...
            try
            {
                ...
                if (strTipoVuelo.Split('#').Length > 1)
                {
                    sNombreSP = "usp_adm_cns_precioticket_sel";
                    strTipoVuelo = strTipoVuelo.Split('#')[0];
                }
        ...
        public TipoTicket validarTipoTicket(string strTipoVuelo, string strTipoPas, string strTipoTrans)
        {
            ...
            try
            {
                Hashtable hsSelectByIdUSP = (Hashtable)htSPConfig["TTSP_VALIDAR_TIPO_TICKET"];
                string sNombreSP = (string)hsSelectByIdUSP["TTSP_VALIDAR_TIPO_TICKET"];
                if (strTipoVuelo.Split('#').Length > 1)
                {
                    sNombreSP = "usp_vta_cns_validartipoticket_sel";
                    strTipoVuelo = strTipoVuelo.Split('#')[0];
                }
                result = base.listarDataReaderSP(sNombreSP, strTipoVuelo, strTipoPas, strTipoTrans);
        ...
        public TipoTicket poblar(IDataReader dataReader)
        {
            ...
            if (dataReader[(string)htSelectAllUSP["Dsc_Moneda"]] != DBNull.Value)
                objTipoTicket.Dsc_Moneda = (string)dataReader[(string)htSelectAllUSP["Dsc_Moneda"]];

            return objTipoTicket;
        }

        public TipoTicket poblar2(IDataReader dataReader)
        {
            Hashtable htSelectAllUSP = (Hashtable)htSPConfig["TTSP_SELECTALL"];
            TipoTicket objTipoTicket = new TipoTicket();
            if (dataReader[(string)htSelectAllUSP["Cod_Tipo_Ticket"]] != DBNull.Value)
