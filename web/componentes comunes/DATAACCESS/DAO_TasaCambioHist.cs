    ...
    public class DAO_TasaCambioHist : DAO_BaseDatos
    {
        ...
        public DataTable obtener(string sCodTasaCambio)
        {
            ...
            strSQL = "SELECT tch.Cod_Tasa_Cambio, tch.Tip_Cambio, tch.Cod_Moneda, tch.Imp_Valor, tch.Cod_Moneda2, " +
                     ...
                     " CASE" +
                     " WHEN  (tu.Ape_Usuario IS NULL AND tu.Nom_Usuario IS NULL) THEN tch.Log_Usuario_Mod " +
                     " ELSE ISNULL(tu.Nom_Usuario,' - ') +', '+ ISNULL(tu.Ape_Usuario, ' - ')" +
                     " END Nom_Usuario, " +
                     " dbo.fnFormatDate (tch.Fch_Ini, 'DD/MM/YYYY hh:xx:ss') Fch_Ini2, " +
                     " dbo.fnFormatDate (tch.Fch_Fin, 'DD/MM/YYYY hh:xx:ss') Fch_Fin2 " +
                     " FROM TUA_TasaCambioHist tch " +
                     " LEFT JOIN TUA_Usuario tu on tch.Log_Usuario_Mod = tu.Cod_Usuario," +
                     " TUA_ListaDeCampos tlc, TUA_Moneda tm" +
