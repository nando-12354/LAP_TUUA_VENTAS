    ...
    public class DAO_Usuario : DAO_BaseDatos
    {
        ...
        public bool insertar(Usuario objUsuario)
        {
            try
            {
                ...
                helper.AddInParameter(objCommandWrapper, "Flg_Destrabe", DbType.String, objUsuario.BGenerarCodeBarDestrabe);
                helper.AddInParameter(objCommandWrapper, "Flg_Molinete", DbType.String, objUsuario.BGenerarCodeBarMolinete);
        ...
        public bool actualizar(Usuario objUsuario)
        {
            try
            {
                ...
                helper.AddInParameter(objCommandWrapper, "Flg_Destrabe", DbType.String, objUsuario.BGenerarCodeBarDestrabe);
                helper.AddInParameter(objCommandWrapper, "Cod_Destrabe", DbType.String, objUsuario.SCodeBarDestrabe);
                helper.AddInParameter(objCommandWrapper, "Flg_Molinete", DbType.String, objUsuario.BGenerarCodeBarMolinete);
                helper.AddInParameter(objCommandWrapper, "Cod_Molinete", DbType.String, objUsuario.SCodeBarMolinete);
...
public bool actualizarContraseña(int NumDias )
        ...
        public Usuario poblar(IDataReader dataReader)
        {
            try
            {
                ...
                if (dataReader[(string)htSelectAllUSP["Emp_Recaudadora"]] != DBNull.Value)
                    objUsuario.Emp_Recaudadora = (string)dataReader[(string)htSelectAllUSP["Emp_Recaudadora"]];
                return objUsuario;
            }
            catch (Exception ex)
            {
        ...
        public Usuario poblar2(IDataReader dataReader)
        {
            try
            {
                Hashtable htSelectAllUSP = (Hashtable)htSPConfig["USP_SELECTALL"];
                ...
                if (dataReader[(string)htSelectAllUSP["Log_Hora_Mod"]] != DBNull.Value)
                    objUsuario.SLogHoraMod = (string)dataReader[(string)htSelectAllUSP["Log_Hora_Mod"]];
                if (dataReader["Flg_Destrabe"] != DBNull.Value)
                    objUsuario.BGenerarCodeBarDestrabe = (string)dataReader["Flg_Destrabe"];
                if (dataReader["Cod_Destrabe"] != DBNull.Value)
                    objUsuario.SCodeBarDestrabe = (string)dataReader["Cod_Destrabe"];
                if (dataReader["Flg_Molinete"] != DBNull.Value)
                    objUsuario.BGenerarCodeBarMolinete = (string)dataReader["Flg_Molinete"];
                if (dataReader["Cod_Molinete"] != DBNull.Value)
                    objUsuario.SCodeBarMolinete = (string)dataReader["Cod_Molinete"];
                if (dataReader[(string)htSelectAllUSP["Emp_Recaudadora"]] != DBNull.Value)
                    objUsuario.SLogHoraMod = (string)dataReader[(string)htSelectAllUSP["Emp_Recaudadora"]];
                return objUsuario;
            }
            catch (Exception ex)
            {
        ...
        public string obtenerNombreCompletoUsuario(string sCodUsuario)
        {
            DataTable dt;

            String strSQL = "SELECT Nom_Usuario + ' ' + Ape_Usuario AS NombreCompleto " +
                            "FROM TUA_Usuario WHERE Cod_Usuario = '" + sCodUsuario + "'";
            dt = base.ListarDataTableQY(strSQL);

            string sNombreCompletoUsuario = sCodUsuario;

            if (dt.Rows.Count > 0)
            {
                sNombreCompletoUsuario = dt.Rows[0]["NombreCompleto"].ToString();
            }
            return sNombreCompletoUsuario;
        }
        //public List<Usuario> ListarUsuariosNivelTres(string nivelCaja, string empresaRecaudadora, string codigoUsuario)
        //{
        //    List<Usuario> usuarios = new List<Usuario>();

        //    using (DbCommand command = helper.GetStoredProcCommand("usp_cns_pcs_listarusuario_nivel3_sel"))
        //    {
        //        command.CommandType = CommandType.StoredProcedure;

        //        helper.AddInParameter(command, "@NivelCaja", DbType.String, nivelCaja);
        //        helper.AddInParameter(command, "@EmpresaRecaudadora", DbType.String, empresaRecaudadora);
        //        helper.AddInParameter(command, "@CodigoUsuario", DbType.String, codigoUsuario);
        ...
        //                {
        //                    SCodUsuario = reader["Cod_Usuario"].ToString(),
        //                    SNomUsuario = reader["Nom_Usuario"].ToString(),
        //                };
        ...
        //    return usuarios; // Asegúrate de que el método retorne la lista de usuarios
        //}
