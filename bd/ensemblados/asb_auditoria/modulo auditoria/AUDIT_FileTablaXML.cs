...
namespace LAP.TUUA.AUDITORIA_CLR
{
      class AUDIT_FileTablaXML
      {
            ...
            public ArrayList selectAllColumnsByTable(String table, SqlConnection sqlCon)
            {
                  ...
                  string sql = "select xml  from  TUA_ParameGeneral ";
                  ...
                  foreach (XmlNode node in elemList)
                  {
                        if (node.SelectSingleNode("NOMBRE_TABLE").InnerText.Equals(table))
                        {
                              ...
                              foreach( XmlNode nodocol in cols)
                              {
                                   ...
                                   columna.Nom_Columna = nodocol.SelectSingleNode("NOMBRE_COL").InnerText;
                                   columna.Nom_Alias = nodocol.SelectSingleNode("ALIAS_COL").InnerText;
                                   columna.Dsc_Tipo = nodocol.SelectSingleNode("TIPO").InnerText;
                                   columna.Ord_ID = nodocol.SelectSingleNode("ORDINAL_ID").InnerText;
                                   //<relacion>
                                   //<options>
                                   columna.Audit_Flag = nodocol.SelectSingleNode("AUDIT_COL").InnerText;
                                   columna.Key_Flag = nodocol.SelectSingleNode("KEY").InnerText;
