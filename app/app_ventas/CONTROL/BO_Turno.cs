...
namespace LAP.TUUA.CONTROL
{
    public class BO_Turno
    {
        ...
        public DataTable ListarTurnosExtornoTicket(string estado, string codigoCajero, string codigoTurno, string fechaInicio, string fechaFin)
        {
            var turnos = objCnxConsulta.ConsultaTurnoxFiltro(fechaInicio, fechaFin, null, "1", "", "", "0");
            var filter = " 1 = 1";

            if (!string.IsNullOrEmpty(codigoTurno))
                filter += " AND Cod_Turno = " + codigoTurno;

            if (estado == "A")
                filter += " AND Isnull(Fch_Fin,'') = ''";
            if (estado == "C")
                filter += " AND Isnull(Fch_Fin,'') <> ''";

            if (!string.IsNullOrEmpty(codigoCajero))
                filter += " AND Cta_Usuario = " + codigoCajero;

            var dv = new DataView(turnos, filter, "", DataViewRowState.CurrentRows);
            return dv.ToTable();
        }

        public DataTable GetTicketsByFilter(string strTicketDesde, string strTicketHasta, string strCodTurno)
        {
            return objCnxConsulta.GetTicketsByFilter(strTicketDesde, strTicketHasta, strCodTurno);
        }

        public DataTable consultarDetalleTicket(string sNumTicket, string sTicketDesde, string sTicketHasta)
        {
            return objCnxConsulta.consultarDetalleTicket(sNumTicket, sTicketDesde, sTicketHasta);
        }

        public DataTable consultarHistTicket(string sNumTicket)
        {
            return objCnxConsulta.consultarHistTicket(sNumTicket);
        }
